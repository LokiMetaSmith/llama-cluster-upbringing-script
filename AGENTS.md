...
Run `npm run lint` or `scripts/lint.sh` to check for formatting and syntax errors. Static analysis (`mypy`) and dead-code detection (`vulture`) are included.
...
* Formatter: Follow the patterns already in neighboring files. Do not hand-format or reformat code you are not otherwise changing. Do not add comments that restate the code.
...
### Pre-commit Hook
Always ensure the custom pre-commit hook is active by running `git config core.hooksPath .githooks`. This hook provides a safety net against logic errors and missing ROCm checks.
...
### Pre-flight Checks (Always Do)
* **Always run the `scripts/agent_preflight.sh` script prior to submitting any code.** This wrapper runs tests, linters, and static analysis tools.
* **Always include the output** (or a statement of its success) when you submit a task. All checks within it must pass.
