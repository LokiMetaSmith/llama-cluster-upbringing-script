...
* The `magic_mirror` Nomad template uses the robust Jinja2 syntax `{{ (ansible_default_ipv4 | default({})).get('address', '127.0.0.1') }}` for the server bind address to prevent rendering errors if the `ansible_default_ipv4` fact is undefined.
* The user prefers Ansible templates to be robust and variables to be defined in configuration files (e.g., `defaults/main.yaml`) rather than hardcoded in tasks.
* The `pipecatapp/start_pipecat.sh` script is dynamically generated by Ansible (`ansible/tasks/build_pipecatapp_image.yaml`) and is excluded from git tracking; it must exist in the local filesystem for Docker builds.
* The `pipecatapp` Ansible role defines `pipecat_image_id` in `ansible/roles/pipecatapp/defaults/main.yaml` (defaulting to `"pipecatapp:local"`) to ensure the variable is accessible to the `expert.nomad.j2` template and to avoid hardcoding within task definitions.
...
* The `magic_mirror` server binary (v0.8.4) accepts the `--bind <HOST:PORT>` argument for network configuration, but configuration via a TOML/JSON file is preferred to define applications.
* Nomad job IDs for `llamacpp-rpc` services may be derived from either the model name or the model filename depending on the configuration version; maintenance scripts must account for both naming conventions.
* Integrating application configuration files directly into Nomad job templates (using the `template` stanza with `change_mode = "restart"`) ensures services automatically restart when configuration changes, avoiding manual handler management in Ansible.
* The `magic_mirror` Nomad job configuration includes `RUST_BACKTRACE = "1"` in the task environment to facilitate debugging of binary crashes.
...
* The `scripts/memory_audit.py` tool queries Prometheus for peak memory usage over 24 hours and can update Nomad job reservations to match actual usage plus a buffer.
* The `scripts/lint.sh` utility validates YAML, Markdown, and Jinja templates (requiring `yamllint`, `markdownlint-cli`, and `djlint` installed) but does not include linters for JavaScript files.
* Unimplemented or future features for the VR interface, such as Spatial Audio, VR Keyboard, and Multi-Room Navigation, are tracked in `pipecatapp/TODO.md`.
* Executing `pipecatapp` tests or server locally requires installing `numpy`, `python-consul2`, `fastapi`, `uvicorn`, `httpx`, `pytest`, `pytest-asyncio`, `pytest-playwright`, `pytest-mock`, `requests`, `pyyaml`, `jinja2`, `websockets`, `paramiko`, `pipecat-ai` (and its dependencies), and setting `PYTHONPATH` to include `pipecatapp`, `pipecatapp/tools`, and `pipecatapp/workflow`.
* Executing tests in this environment requires using `python -m pytest` and setting `PYTHONPATH` to include the project root, `pipecatapp`, `pipecatapp/tools`, and `pipecatapp/workflow` to correctly resolve all internal module imports.
...
* The `RateLimiter` class in `pipecatapp/rate_limiter.py` prevents memory leaks by performing lazy cleanup of expired IP keys during request processing (`__call__`), controlled by a `cleanup_interval`.
* To resolve circular import issues between `app.py` and `web_server.py`, `YOLOv8Detector` imports `web_server` locally within the `process_frame` method to access the WebSocket manager.
* The `expert_tracker.py` module integrates with the memory graph service using the tool name `create_relationship` (wrapping `handle_create_relationship`) with arguments `from_memory_id`, `to_memory_id`, and `relationship_type`.
* The `YOLOv8Detector` in `pipecatapp/app.py` broadcasts visual debug frames as base64-encoded JPEGs over WebSockets using the JSON structure `{'type': 'vision_debug', 'data': img_base64}`.
* The `memory-graph-service` source code resides in `pipecatapp/memory_graph_service/`; `server.py` defines the FastMCP instance as `app` to be compatible with the Dockerfile's `uvicorn server:app` command.
* The `pipecatapp` Python environment requires the `mcp` package to interact with the memory graph service; `expert_tracker.py` treats it as an optional import to handle potential missing dependencies.
* The `SSH_Tool` (`pipecatapp/tools/ssh_tool.py`) enforces host key verification using `paramiko.RejectPolicy()` and loads system host keys; connection targets must be present in the `known_hosts` file to prevent Man-in-the-Middle attacks.
* The `/api/cluster/metrics` endpoint queries Prometheus for `nomad_client_allocs_memory_usage` and `nomad_client_allocs_cpu_total_ticks` to populate the cluster visualization.
* The `get_cluster_metrics` function in `pipecatapp/web_server.py` executes CPU and Memory Prometheus queries in parallel using `asyncio.gather` and a dedicated global `metrics_client` to optimize response time and separate concerns.
* The `memory-graph` service runs as a standalone Docker container (`memory-graph-service`) utilizing the `memorygraphMCP` library, communicating via MCP over SSE on port 8000.
* The `ServiceDiscoveryCache` in `pipecatapp/web_server.py` relies on the atomic nature of synchronous Python operations within the asyncio loop, avoiding `asyncio.Lock` to maximize cache throughput.
* The `magic_mirror` server binary (v0.8.4) requires a GPU with Vulkan/DRM support (specifically a render node like `/dev/dri/renderD*`) and fails to start if only software rasterizers (e.g., `llvmpipe`) are available.
* The `magic_mirror` server binary (v0.8.4) requires TLS configuration when binding to non-private addresses (like `0.0.0.0`). For non-TLS setups, it must be bound to a specific private IP address (e.g., `127.0.0.1` or a LAN IP).
* The `pipecatapp` web server includes a `/cluster` endpoint serving `cluster.html` for interactive visualization, and an `/api/cluster/metrics` endpoint that aggregates CPU and Memory usage from Prometheus.
* The `start_pipecat.sh` script (entrypoint for `pipecatapp`) is configured to source environment variables from `/opt/pipecatapp/pipecat.env` if the file exists, facilitating configuration in containerized environments.
* The `memory-graph` service implementation uses the `memorygraphMCP` PyPI package (imported as `memorygraph`), wrapping tool handlers like `handle_store_memory` in a `FastMCP` server.
...
* The `pipecatapp` WebVR interface (`vr_index.html`) integrates `litegraph.js` into an A-Frame scene using `aframe-htmlembed-component` and handles voice input via the browser's Web Speech API.
* Real-time log broadcasts in `pipecatapp` via WebSockets utilize the JSON structure `{'type': 'log', 'data': log_entry}`.
* The `pipecatapp` frontend (`terminal.js`) implements a command history feature (max 50 entries) accessible via Up/Down arrow keys.
* Navigation to the VR interface is accessible via 'Enter VR' buttons added to the controls section of `index.html` and the toolbar of `workflow.html`.
* The `pipecatapp` cluster visualization (`cluster.html`) requires specific static node definitions (e.g., `pipecat_app`, `vllm`, `postgres`) in `SERVICE_DATA` to support the initial 'Genesis' animation sequence.
* The 'Cluster Online' message in the visualization is styled to be discrete (opacity 0.8, font-size 2rem) per user preference.
* Voice commands captured in the VR interface are transmitted to the backend via the existing WebSocket connection using the JSON payload `{'type': 'user_message', 'message': transcript}`.
* The `WorkflowEditor.init` function in `pipecatapp/static/js/editor.js` supports an optional `skipResize` flag to prevent window resize event listeners from overriding fixed canvas dimensions in VR environments.
* The `pipecatapp` frontend (`terminal.js`) enforces input validation for 'Save/Load State' buttons, disabling them via the `disabled` attribute when the input is empty.
* The VR interface (`vr_index.html`) implements a 'Focus Mode' (triggered by voice commands like 'focus' or 'blackout') that utilizes A-Frame raycasting to detect the active element and reduces the opacity of non-focused panels.
* The `pipecatapp` frontend CSS (`index.html`) defines `button:disabled` styles with `opacity: 0.5` and `cursor: not-allowed`.
