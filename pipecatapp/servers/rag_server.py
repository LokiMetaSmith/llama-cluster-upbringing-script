...
_rag_instance = RAG_Tool(base_dir="/opt/pipecatapp", allowed_root="/opt/pipecatapp")

@mcp.tool()
async def scan_directory(directory: str) -> str:
    """Scans a directory for documents to build the knowledge base."""
    return await _rag_instance.scan_directory(directory)

@mcp.tool()
async def search(query: str, k: int = 5) -> str:
    """Searches the knowledge base for relevant information."""
    return await _rag_instance.search(query, k)

@mcp.tool()
async def add_document(filepath: str) -> str:
    """Adds a single document to the knowledge base."""
    return await _rag_instance.add_document(filepath)

@mcp.tool()
async def search_knowledge_base(query: str, k: int = 5) -> str:
    """Searches the explicitly loaded knowledge base."""
    return await _rag_instance.search_knowledge_base(query, k)

@mcp.tool()
async def set_scope(path: str) -> bool:
    """Sets the search scope."""
    return await _rag_instance.set_scope(path)
