...
_document_instance = DocumentTool(backend_config={"type": "local", "directory": "/opt/pipecatapp"})

@mcp.tool()
def search(query: str) -> str:
    """Searches the document backend."""
    return _document_instance.search(query)

@mcp.tool()
def get_text(document_id: str) -> str:
    """Retrieves the full text of a specific document."""
    return _document_instance.get_text(document_id)

@mcp.tool()
def add_bookmark(document_id: str, document_title: str, text_block: str = "", note: str = "") -> str:
    """Saves a reference to a document."""
    return _document_instance.add_bookmark(document_id, document_title, text_block, note)

@mcp.tool()
def list_bookmarks() -> str:
    """Lists all saved document bookmarks."""
    return _document_instance.list_bookmarks()
