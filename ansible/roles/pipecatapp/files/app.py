...
class TwinService(FrameProcessor):
    ...
    async def process_frame(self, frame, direction):
        ...
        try:
            ...
            # In this initial refactor, we are not implementing the agent "loop" yet.
            # We will run the workflow once. If it produces a final text response, we send it.
            # If it produces a tool call, we will handle that in a future step.
            final_response = await workflow_runner.run(global_inputs)
            ...
            if final_response:
                ...
                # This path is taken if the workflow ends on a tool call.
                # The full loop implementation will handle this.
                logging.warning("Workflow did not produce a final response. Tool loop not yet implemented.")
                await self._send_response("I need to use a tool, but the multi-turn agent loop is not yet implemented in the new workflow engine.")
